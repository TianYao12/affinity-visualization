...
import type { AnalysisStage } from "@/types/prediction";
...
const getStageIcon = (stageId: AnalysisStage["id"]) => {
  switch (stageId) {
    case "esm_embedding":
      return <Dna className="w-5 h-5" />;
    case "chemgan_generation":
...
export default function AnalysisPipeline({
  ...
  const getStepIndex = (stepId: string) =>
    staticSteps.findIndex((s) => s.id === stepId);
  const currentStepIndex = getStepIndex(currentStep);
  const hasRemoteStages = Boolean(stages?.length);
  const remoteSteps =
    stages?.map((stage) => ({
      id: stage.id,
      icon: getStageIcon(stage.id),
      title: stage.label,
      description: stage.details[0] || "Processing stage",
      details: stage.details,
      status: stage.status,
    })) ?? [];
  const firstIncompleteRemoteIndex = remoteSteps.findIndex(
      ...
      <div className="space-y-4">
        {hasRemoteStages
          ? remoteSteps.map((step, index) => {
              const activeIndex =
                firstIncompleteRemoteIndex === -1
                  ? remoteSteps.length - 1
                  : firstIncompleteRemoteIndex;
